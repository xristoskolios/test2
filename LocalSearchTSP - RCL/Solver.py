...
class Solver:
    ...
    def solve(self):
        ...
        for i in range(100):
            ...
            self.ApplyNearestNeighborMethod(i, rcl_size)
            construction_cost = self.sol.cost
            self.LocalSearch(2)
            SolDrawer.draw(i, self.sol, self.allNodes)
            print(i, 'Construction: ', construction_cost, ' LS:', self.sol.cost)
    ...
    def ApplyNearestNeighborMethod(self, sd, rcl_size):
        self.sol = Solution()
        ...
        for i in range(0, len(self.customers)):
            ...
            cust_index = rcl[tup_index][1]
            insertedCustomer = self.customers[cust_index]
